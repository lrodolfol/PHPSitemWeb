...
class Email {
     ...
     * @return bool
     */
    public function sendEmailphp($fromEmail = CONF_MAIL_SENDER['address'], $fromName = CONF_MAIL_SENDER["name"]): bool {
        $emailDestinatario = $this->data->toEmail;

        $emailRemetente = $fromEmail;
        $assuntoEmail = $this->message;

        return true;
        $mensagem = "Olá " . $this->data->toName . $this->message;
        $headers = 'From:' . $emailRemetente . "\r\n" .
                'Reply-To:' . $emailRemetente . "\r\n" .
                'X-Mailer: PHP/' . phpversion();
        $envio = mail($emailDestinatario, $assuntoEmail, $mensagem, $headers);
        if (!$envio) {
            $this->message = "Erro ao enviar email php";
            return false;
        } else {
            return true;
        }
    ...
    //AGENDA DISPARO DE EMAIL NO BANCO DE DADOS
